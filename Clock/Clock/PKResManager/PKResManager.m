...
{
    if ([_styleId isEqualToString:name]
        || name == nil )
    {
...
{
    return [self previewImageByStyleName:_styleId];
}
...
- (NSBundle *)bundleByStyleName:(NSString *)name
{
    ...
    BOOL changeStyle = NO;
    if ([self.styleId isEqualToString:name])
    {
        changeStyle = YES;
    }
...
//    label.shadowOffset = [self shadowOffSetForKey:key];
...
//    [button.titleLabel setShadowOffset:[self shadowOffSetForKey:key]];
